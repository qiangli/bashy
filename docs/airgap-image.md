...
| coreutils | `ar` | not usable offline | bin-managed external: would download — ar: posix provider cache root: no OS account record for uid 0: open /e |
...
| coreutils | `ctags` | not usable offline | bin-managed external: would download — ctags: posix provider cache root: no OS account record for uid 0: open |
...
| coreutils | `ex` | not usable offline | bin-managed external: would download — ex: posix provider cache root: no OS account record for uid 0: open /e |
...
| coreutils | `localedef` | not usable offline | bin-managed external: would download — localedef: posix provider cache root: no OS account record for uid 0:  |
| coreutils | `logger` | works | logger (qiangli/coreutils) <ver> |
| coreutils | `logname` | works | logname (qiangli/coreutils) <ver> |
| coreutils | `lp` | not usable offline | bin-managed external: would download — lp: posix provider cache root: no OS account record for uid 0: open /e |
| coreutils | `ls` | works | ls (qiangli/coreutils) <ver> |
| coreutils | `m4` | not usable offline | bin-managed external: would download — m4: posix provider cache root: no OS account record for uid 0: open /e |
...
| coreutils | `nm` | not usable offline | bin-managed external: would download — nm: posix provider cache root: no OS account record for uid 0: open /e |
...
| coreutils | `strip` | not usable offline | bin-managed external: would download — strip: posix provider cache root: no OS account record for uid 0: open |
...
| coreutils | `vi` | not usable offline | bin-managed external: would download — vi: posix provider cache root: no OS account record for uid 0: open /e |
...
| coreutils | `why` | not usable offline | bin-managed external: would download — why: resolve witr: binmgr: fetch release pranshuparmar/witr@<ver>: Ge |
...
| shell | `echo $HOME (non-tty stdout)` | stated | prints $HOME: Stage 0 output canonicalization (output_reduce.go); a tty passes through |
| shell | `network (curl)` | absent | no network tools in the image; --network=none besides |
| verbs | `act` | not usable offline | bin-managed external: would download — Error: act: resolve: binmgr: fetch release nektos/act@latest: Get "htt |
| verbs | `act-runner` | not usable offline | bin-managed external: would download — act-runner runs Gitea's act_runner  downloaded, sha256-verified, and c |
| verbs | `activity` | works | usage: bashy activity {status\|subscribe\|unsubscribe\|interests\|tail\|sho |
...
| verbs | `cargo` | not usable offline | engine / remote-by-design (by design): Error: rust: fetch sha256 sidecar: Get "https://static.rust-lang.org/r |
...
| verbs | `clang` | not usable offline | engine / remote-by-design (by design): Error: clang: no system clang on linux/<arch>  install Xcode Command Li |
| verbs | `cmake` | not usable offline | engine / remote-by-design (by design): Error: cmake: fetch checksums: Get "https://github.com/Kitware/CMake/r |
| verbs | `coach` | works | Coach starts a steerable session, watches for a doomed loop, and when  |
| verbs | `command` | works | bash builtin |
| verbs | `commands` | works | usage: commands [COMMAND] [-v] [--json\|--plain\|--agentic\|--all\|--gnu\|- |
...
| verbs | `curl` | not usable offline | engine / remote-by-design (by design): Error: curl: not found on PATH (install it, or use `bashy fetch`) |
...
| verbs | `dks` | not usable offline | engine / remote-by-design (by design): bashy dks: not available in this build  the DKS VM provisioner needs t |
| verbs | `docker` | not usable offline | engine / remote-by-design (by design): bashy podman: fetching podman <ver> for linux/<arch>  first run only |
| verbs | `doctl` | not usable offline | bin-managed external: would download — Error: doctl: binmgr: fetch release digitalocean/doctl@: Get "https:// |
| verbs | `doctor` | works | usage: doctor [--json] |
| verbs | `foreman` | works | foreman <ver> |
| verbs | `full` | present | runs in-process; --version/--help rc=2: bashy full: line 1: command: --: invalid option |
| verbs | `gate` | works | gate runs the project's gate  the command that decides pass/fail  and |
| verbs | `gcloud` | not usable offline | engine / remote-by-design (by design): Error: gcloud: gcloud not found on PATH  install the Google Cloud SDK  |
| verbs | `gh` | not usable offline | bin-managed external: would download — Error: gh: resolve: binmgr: fetch release cli/cli@latest: Get "https:/ |
| verbs | `git` | not usable offline | bin-managed external: would download — Error: gitscm: no system git on PATH (unix hosts use the platform git) |
| verbs | `git-scm` | not usable offline | bin-managed external: would download — Error: gitscm: no system git on PATH (unix hosts use the platform git) |
| verbs | `go` | not usable offline | engine / remote-by-design (by design): Error: gotoolchain: fetch release index: Get "https://go.dev/dl/?mode= |
| verbs | `handoff` | works | handoff captures everything a successor needs and passes the work on. |
| verbs | `helm` | not usable offline | engine / remote-by-design (by design): Error: helm: resolve latest version: Get "https://api.github.com/repos |
...
| verbs | `kopia` | not usable offline | bin-managed external: would download — kopia runs the Kopia repository server  downloaded, sha256-verified, a |
| verbs | `kubectl` | not usable offline | engine / remote-by-design (by design): Error: kubectl: resolve stable version: Get "https://dl.k8s.io/release |
| verbs | `lexicon` | works | lexicon is the project's jargon, projected from the registries that al |
| verbs | `login` | not usable offline | engine / remote-by-design (by design): tessaro: this machine isn't connected to Tessaro yet. |
| verbs | `loom` | not usable offline | bin-managed external: would download — loom runs Gitea  downloaded, sha256-verified, and cached by binmgr (no |
...
| verbs | `rclone` | not usable offline | bin-managed external: would download — Error: rclone: resolve: binmgr: fetch release rclone/rclone@latest: Ge |
...
| verbs | `rg` | not usable offline | bin-managed external: would download — Error: rg: binmgr: fetch release BurntSushi/ripgrep@: Get "https://api |
| verbs | `run` | present | runs in-process; --version/--help rc=2: bashy run: unknown option "--help" |
| verbs | `rust` | not usable offline | engine / remote-by-design (by design): Error: rust: fetch sha256 sidecar: Get "https://static.rust-lang.org/r |
| verbs | `rustc` | not usable offline | engine / remote-by-design (by design): Error: rust: fetch sha256 sidecar: Get "https://static.rust-lang.org/r |
| verbs | `rustup` | not usable offline | engine / remote-by-design (by design): Error: rust: fetch sha256 sidecar: Get "https://static.rust-lang.org/r |
| verbs | `sandbox` | not usable offline | engine / remote-by-design (by design): bashy podman: fetching podman <ver> for linux/<arch>  first run only |
...
| verbs | `seaweedfs` | not usable offline | bin-managed external: would download — seaweedfs runs SeaweedFS  downloaded, sha256-verified, and cached by b |
...
| verbs | `transpile` | present | runs in-process; --version/--help rc=2: transpile: unknown flag: --help |
| verbs | `upgrade` | works | bashy self fetches and caches a released bashy binary using the same |
| verbs | `uv` | not usable offline | engine / remote-by-design (by design): Error: python/uv: fetch sha256 sidecar: Get "https://github.com/astral |
