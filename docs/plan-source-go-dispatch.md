...
| `--go-checker-branch-errors` | Leave label/goto/break/continue errors to the Go type checker instead of gc's syntax-stage branch check (the types2/go-types test environment). Requires `--source=go`. |
| `--go-check-after-syntax-errors` | Type-check the partial AST after a Go syntax rejection and report both parse and checker diagnostics (the go/types and types2 check_test environment); default is gc's: a syntax rejection is the complete result. Requires `--source=go`. |
...
| `transpile --go-native-unit` | Emit one Go package with its original declarations, imports and initialization. Requires `--source=go --go-import-path PATH`; explicit `--go-package` entries supply checker dependencies. Incompatible with `--go-library` and `--go-test-main`. |
