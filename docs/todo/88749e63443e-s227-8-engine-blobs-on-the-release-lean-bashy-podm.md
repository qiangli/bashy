...
**DECIDED (operator, 2026-09-20): pinned engine tag — option (b).** Blobs live on a dedicated rolling release tag (`engines-v1`), read by `fetchReleaseGz` when `BASHY_ENGINE_TAG` (default `engines-v1`, beside the existing `BASHY_ENGINE_REPO`) is set; unset/empty keeps today's latest-release lookup for anything that still relies on it. Blobs are rebuilt only when podman/gvproxy/vfkit is bumped, never per bashy release; a blob build can never gate a bashy release. Windows: no blob.

1. DECISION (recorded above): where the lean binary looks for blobs. (a) every release re-publishes blobs — a podman-from-source build per release, the thing that hit the 24 h cap; or (b) a dedicated rolling engine release tag (e.g. `engines-v1`, `BASHY_ENGINE_TAG` override next to the existing `BASHY_ENGINE_REPO`) that `fetchReleaseGz` reads when set — blobs are rebuilt only when podman is bumped. Recommendation: (b); podman moves independently of bashy, and a blob build failing must never gate a bashy release (the workflow header already says so).
