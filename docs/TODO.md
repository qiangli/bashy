...
**VSC-PCTS UTILITIES campaign (the active cert front, 2026-07-17)** — publication consent for utilities-suite results was granted 2026-07-29 (ticket #280298), for conformance-work purposes on the same terms as the shell arm. The 2026-07-08 sweep (bashy userland 2551P/912F vs GNU 2947P/516F = 86.6%, with the concentrated per-command delta) is now published in `vsc-pcts-run-status.md` — tallies and assertion identifiers belong there, not in this progress list. The `pkg/bre` fixes below landed AFTER that scoring and have not been re-measured. Raw journals and private run-record pointers still stay out of this public repo, and no "certified"/"passes" claim is ever covered.
