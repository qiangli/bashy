...
func TestE2ERegisteredScriptSchema(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{[]string{"paint", "--count", "007", "blue"}, "entered|--mode=fast|--count=7|blue|3\n", 0},
		{[]string{"paint", "--count", "nope", "red"}, "", 2},
		{[]string{"paint", "red"}, "", 2},
		{[]string{"legacy0", "--unknown", "raw"}, "entered|--unknown|raw\n", 0},
	} {
		out, stderr, code := runBashyStdEnv(bin, env, tc.args...)
		if out != tc.out || code != tc.code {
			t.Errorf("front door %v: out=%q stderr=%q code=%d", tc.args, out, stderr, code)
		}
	}
