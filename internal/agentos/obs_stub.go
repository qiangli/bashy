...
// k8s/aws — a mesh-HOST concern — so the default worker binary omits it. Build a
// host binary with `-tags bashy_obs`, or run otel on a host node in the mesh.
func dispatchObs(arg string) {
	if arg == "otel" {
		fmt.Fprintln(os.Stderr,
			"bashy otel: the observability stack is not in this build "+
				"(rebuild with -tags bashy_obs, or run it on a host node in the mesh)")
		os.Exit(1)
	}
}
