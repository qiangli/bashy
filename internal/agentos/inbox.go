...
pretends such a session was adopted.`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			reader, err := bus.BoardIdentity(as)
			if err != nil {
				return err
			}
...
func snapshotUnifiedInbox(reader string, limit int, includeBus bool) (inboxBatch, error) {
	...
	for _, room := range rooms {
		...
		for _, record := range events {
			e := record.Event
			out = append(out, unifiedInboxEvent{Schema: unifiedInboxSchema, Source: "meet", Seq: record.Seq, At: e.TS.Format(time.RFC3339Nano), From: e.Speaker, To: e.To, Topic: e.Kind, Room: room.ID, Body: e.Text})
		}
		id := room.ID
