...
func TestDefineSessionRealShim(t *testing.T) {
	var out strings.Builder
	script := defineSessionScript + "define() { command " + shellQuote(bashySelfPath()) + " define \"$@\"; }; define n\n"
	prog, err := syntax.NewParser(syntax.Variant(syntax.LangBashPP)).Parse(strings.NewReader(script), "shim.sh")
	if err != nil {
		t.Fatal(err)
	}
	r, err := interp.New(interp.Lang(syntax.LangBashPP), interp.StdIO(nil, &out, io.Discard), interp.ExecHandlers(defineSessionHandler()))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(out.String(), "value: 42") {
		t.Fatalf("shim output: %q", out.String())
	}
}
...
func TestDefineSessionCandidateNonSelfInvocation(t *testing.T) {
	...
	if name, ok := defineSessionCandidate([]string{bashySelfPath(), "define", "n"}); !ok || name != "n" {
