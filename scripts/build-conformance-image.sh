...
bash53)
  ...
  GOOS=linux GOARCH="$ARCH" CGO_ENABLED=0 "${go_cmd[@]}" build -trimpath -o "$CTX/bin/bash-linux-$ARCH/bash" ./cmd/bash
  GOOS=linux GOARCH="$ARCH" CGO_ENABLED=0 "${go_cmd[@]}" build -trimpath -o "$CTX/bin/bash53suite-linux-$ARCH" ./tools/bash53suite
...
yash)
  ...
  GOOS=linux GOARCH="$ARCH" CGO_ENABLED=0 "${go_cmd[@]}" build -trimpath -o "$CTX/bin/bash-linux-$ARCH/bash" ./cmd/bash
  GOOS=linux GOARCH="$ARCH" CGO_ENABLED=0 "${go_cmd[@]}" build -trimpath -o "$CTX/bin/yashsuite-linux-$ARCH" ./tools/yashsuite
  ...
  log "baked $kept shell-only -p fixtures (manifest chunk_count=$("${go_cmd[@]}" run ./tools/yashsuite --chunks-manifest yash-chunks.json --chunk-count))"
...
if on_windows; then
  ...
  quoted_build_args=""
  [ -z "${BASE_IMAGE:-}" ] || quoted_build_args="--build-arg BASE_IMAGE='$BASE_IMAGE'"
  $OCI machine ssh "$VMM" "cd '$vmctx' && podman build $SQUASH_FLAG $quoted_build_args --platform linux/$ARCH -t '$IMAGE' -f Containerfile ." 2>&1 | grep -iE 'STEP|COMMIT|Successfully|error' | tail -4
else
  $OCI build $SQUASH_FLAG "${BUILD_ARGS[@]}" --platform "linux/$ARCH" -t "$IMAGE" -f "$CTX/Containerfile" "$CTX" 2>&1 | grep -iE 'STEP|COMMIT|Successfully|error' | tail -4
fi
