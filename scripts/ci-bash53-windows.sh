...
# The workflow provisions real glibc locale data in WSL in the preceding step.
# A non-CI Windows run gets the same contract by running the provisioner first.
if [ -z "${BASHY_HOST_LOCALE:-}" ] && [ -f bin/windows-wsl-locales.env ]; then
  # shellcheck disable=SC1091 -- generated by the repository provisioner.
  . bin/windows-wsl-locales.env
fi
...
if [ -n "${BASHY_WSL_DISTRO:-}" ]; then
  ...
  for wanted in en_US.UTF-8 zh_TW.big5 ja_JP.SJIS fr_FR.ISO8859-1 de_DE.UTF-8 zh_HK.big5hkscs ru_RU.CP1251; do
    if ! printf '%s\n' "$available_locales" | grep -Fqix "$wanted"; then
      echo "bash53-windows: provisioned locale is not serviceable through yoke: $wanted" >&2
      exit 2
    fi
  done
fi
